...
func TestParseOptions(t *testing.T) {
	...
	if len(options.FailOnWarningCodes) != 2 || options.FailOnWarningCodes[0] != "WARN_FOLLOW_IMPORTS_HEALTH_STALE" || options.FailOnWarningCodes[1] != "WARN_READINESS_RUN_SLOW" {
...
func TestWriteReadinessJSONIncludesDoctorAndSmokeSummaries(t *testing.T) {
	...
	if len(decoded.AllWarningCodes) != 4 || decoded.AllWarningCodes[2] != "WARN_FOLLOW_IMPORTS_POLL_CATCHUP" || decoded.WarningPolicyFailed {
...
func TestRunReadinessCheckKeepGoingContinuesAfterDoctorFailure(t *testing.T) {
	...
	runner := func(_ context.Context, _ string, args ...string) (string, string, error) {
		...
		case "run ./cmd/codex-mem doctor --json":
			return "", "doctor exploded\n", errors.New("exit status 1")
		case "run ./scripts/mcp-smoke-test":
			return "mcp smoke test passed\n", "", nil
		case "run ./scripts/http-mcp-smoke-test":
			return "http mcp smoke test passed\n", "", nil
		default:
...
func TestRunReadinessCheckFailsOnMatchedWarningCode(t *testing.T) {
	options := readinessOptions{FailOnWarningCodes: []string{"WARN_FOLLOW_IMPORTS_HEALTH_STALE"}}
	runner := func(_ context.Context, _ string, args ...string) (string, string, error) {
		switch strings.Join(args, " ") {
		case "run ./cmd/codex-mem doctor --json":
	...
	if report.Summary != "warning policy failed: WARN_FOLLOW_IMPORTS_HEALTH_STALE" {
		t.Fatalf("summary mismatch: %q", report.Summary)
	}
	if !report.WarningPolicyFailed {
		t.Fatalf("expected warning policy failure, got %+v", report)
	}
	if len(report.MatchedWarningCodes) != 1 || report.MatchedWarningCodes[0] != "WARN_FOLLOW_IMPORTS_HEALTH_STALE" {
		t.Fatalf("unexpected matched warning codes: %+v", report.MatchedWarningCodes)
	}
	if len(report.AllWarningCodes) != 1 || report.AllWarningCodes[0] != "WARN_FOLLOW_IMPORTS_HEALTH_STALE" {
