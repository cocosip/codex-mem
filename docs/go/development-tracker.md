...
- Blockers: none new.
- Next step: Publish tagged GitHub Releases with per-platform archives and a SHA256SUMS manifest, while deferring signatures until signing-key management exists.

### 2026-03-13 Session Update

- Completed: Updated `scripts/package-release` to emit a `SHA256SUMS` manifest for all packaged archives; updated the GitHub Actions `build-release` workflow to publish GitHub Release assets on `v*` tags; documented the decision to ship release assets plus checksums now and defer signatures until signing-key management is available.
- In progress: Tagged-release publication and release-note polish follow-up.
- Blockers: none new.
- Next step: Decide whether detached signatures should be added later, and whether autogenerated GitHub release notes need project-specific customization.
